...
class DDPG:
    ...
    def __init__(self, sess, env, config):
        ...
        if config.LOAD_DIR:
            self.restore(sess, self.env, config, directory=config.LOAD_DIR)
        else:
    ...
    def restore(self, sess, env, config, directory):
        """
        ...
        self.saver.restore(sess, directory)
        ...
        self.critic = CriticNetwork(sess, state_dim, action_dim, config.CRITIC_LEARNING_RATE, config.L2_DECAY, config.TAU, restore=True, seed=config.RANDOM_SEED)
    ...
    def train(self):
        ...
        for i in range(config.MAX_EPISODES):
            ...
            if i % 50 == 0 and i != 0:
                save_start = time()
                save_model = os.path.join(config.SAVE_DIR, "ddpg_model")
                save_buffer = os.path.join(config.SAVE_DIR, "replay_buffer.pkl")
                save_metagraph = os.path.join(config.SAVE_DIR, "ddpg_model.meta")
